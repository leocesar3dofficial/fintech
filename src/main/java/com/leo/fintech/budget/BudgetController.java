...
public class BudgetController {
    ...
    @GetMapping
    public List<BudgetDto> getAllBudgets(@AuthenticationPrincipal JwtUserPrincipal userPrincipal) {
    ...
    public ResponseEntity<BudgetDto> createBudget(@Valid @RequestBody BudgetDto dto,
        ...
        UUID userId = UUID.fromString(userPrincipal.getUserId());
        BudgetDto created = budgetService.createUserBudget(dto, userId);
    ...
    public ResponseEntity<BudgetDto> getBudgetById(@PathVariable("id") Long id,
            @AuthenticationPrincipal JwtUserPrincipal userPrincipal) {
        UUID userId = UUID.fromString(userPrincipal.getUserId());
        BudgetDto budget = budgetService.getUserBudgetById(id, userId);
    ...
    public ResponseEntity<BudgetDto> updateBudget(
            ...
            @Valid @RequestBody BudgetDto dto, @AuthenticationPrincipal JwtUserPrincipal userPrincipal) {
        try {
            UUID userId = UUID.fromString(userPrincipal.getUserId());
            BudgetDto updated = budgetService.updateBudget(id, dto, userId);
    ...
    public ResponseEntity<Void> deleteBudgetSoft(@PathVariable("id") Long id, 
            @AuthenticationPrincipal JwtUserPrincipal userPrincipal) {
        UUID userId = UUID.fromString(userPrincipal.getUserId());
        boolean deleted = budgetService.deleteBudgetIfExists(id, userId);
